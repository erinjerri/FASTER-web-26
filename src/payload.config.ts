...
import { Profiles } from './collections/Profiles.ts'
...
const configuredDbURL = (process.env.DATABASE_URL || process.env.MONGODB_URI || '').trim()
const dbURL =
  configuredDbURL ||
  (process.env.NODE_ENV === 'development' ? 'mongodb://127.0.0.1/faster-platform' : '')
...
export default buildConfig({
  ...
  email: nodemailerAdapter({
    ...
    defaultFromName: process.env.SITE_NAME ?? 'FASTER',
